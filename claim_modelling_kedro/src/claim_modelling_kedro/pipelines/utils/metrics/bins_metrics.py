...
from claim_modelling_kedro.pipelines.p01_init.metric_config import BinsMetricType, MAX_ABS_BIAS_BINS, \
    MAX_UNDERPRICING_BINS, MAX_OVERPRICING_BINS
...
class MaxOverpricingBinsMetric(BinsMetric):
    def __init__(self, config: Config, n_bins: int, **kwargs):
        super().__init__(config=config, n_bins=n_bins, **kwargs)

    def eval(self, group_stats_df: pd.DataFrame) -> float:
        return group_stats_df.mean_overpricing.max()

    def _get_name(self) -> str:
        return f"Max Overpricing {self.n_bins}-Bins Metric"

    def _get_short_name(self) -> str:
        return f"MOP_{self.n_bins}B"

    def get_enum(self) -> BinsMetricType:
        return MAX_OVERPRICING_BINS(n_bins=self.n_bins)

    def is_larger_better(self) -> bool:
        return False


class MaxUnderpricingBinsMetric(BinsMetric):
    def __init__(self, config: Config, n_bins: int, **kwargs):
        super().__init__(config=config, n_bins=n_bins, **kwargs)

    def eval(self, group_stats_df: pd.DataFrame) -> float:
        return group_stats_df.mean_underpricing.max()

    def _get_name(self) -> str:
        return f"Max Underpricing {self.n_bins}-Bins Metric"

    def _get_short_name(self) -> str:
        return f"MUP_{self.n_bins}B"

    def get_enum(self) -> BinsMetricType:
        return MAX_UNDERPRICING_BINS(n_bins=self.n_bins)
